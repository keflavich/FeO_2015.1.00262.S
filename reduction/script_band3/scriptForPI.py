...
# $Id: scriptForPI.py,v 1.19 2017/06/02 09:30:46 dpetry Exp $
...
tmppipe.close()
tmppipe = os.popen("du -smc ../raw | grep total | tail -n 1 | cut -f1")
...
for asdmname in scriptasdms:
    ...
    if not globals().has_key("USEMS"):
        if not os.path.exists(asdmname+'.calibration'):
